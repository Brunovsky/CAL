...
void generateRandomConstX(int n, vector<Point> &vp) {
	...
	int r = rand() % (n-1);
	vp.push_back(Point(0,r));
	vp.push_back(Point(0,r+1));
	int y = 0;
	for (int i = 0; i < n; i++) {
...
int testNP(string name, vector<Point> & pontos, double dmin, NP_FUNC func, string alg) {
	...
	ASSERT_EQUAL_DELTA(dmin, res.dmin, 0.01);
	return nTimeElapsed;
}
...
void testNearestPoints(NP_FUNC func, string alg) {
	...
	if (testNPRand(0x200000, "Pontos2M",  1.0, func, alg) > maxTime)
		return;
	if (testNPRand(0x200000, "Pontos4M",  1.0, func, alg) > maxTime)
		return;
}
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testNP_BF_SortedX));
